...
for (let i = 0; i < elements.length; i++) {
  ...
  //This is the function that is called when the event happens.

  if (header_row_cross[i] !== "" && typeof header_row_cross[i] !== "undefined") {
    header_row_cross[i].addEventListener("click", set_default_colors);
  ...
  function save(elements, i, ccolor) {
    ...
    if (ccolor == elements[i][3]) {
      if (theme.length == 0 || typeof theme.length == "undefined") {
        chrome.storage.sync.get(elements[i][1], function (data) {
          function isEmpty(data) {
            for (var prop in data) {
              if (data.hasOwnProperty(prop))
      ...
      //Checks if there is a theme applied. For some reason the colors are reset here when a theme is applied. Therefor we check if there is no theme applied, them we can continue.
    }
...
function process_data(event) {
  ...
  let all_lines = csv.split("\n");
  //Split the rows by looking for new lines.

  for (let n = 1; n < (all_lines.length - 1); n++) {
    let row = all_lines[n].split(";");
    //Split every line into array row, by looking for ;'s. We now get the cells.

    let cells = [];
    for (let o = 0; o < row.length; o++) {
      cells.push(row[o]);
      //Adds the cell to the array of cells for that row.

    }
    //Loop through every cell in the row.

    theme.push(cells);
    //Pushes the row with cells to the array.
  ...
  function set_default_colors_general(style_to_apply, element_type) {
    for (let l = 0; l < element_type.length; l++) {
      let element = eval("element_type[l][2]");
      //Element is now a Node instead of a string.
  ...
  //Define the function that will set the default colors for the elements.
  set_default_colors_general("backgroundColor", background_elements);
  set_default_colors_general("color", text_elements);
  //Calls the function for both the background elements and the text elements.

  for (let q = 0; q < theme.length; q++) {
    for (let i = 0; i < elements.length; i++) {
      if (theme[q][0] == elements[i][0]) {

        let color = "#" + theme[q][1];
        //Var color gets the color directly from the theme.
        ...
        chrome.tabs.query({
          ...
          //Sets the colors from the theme to the element(s) in the DOM.
        });
        //Sets the colors directly to the DOM.
        ...
        document.getElementsByName(elements[i][1])[0].value = color;
        //Sets the color to the input box.
      }
      //Check if the Names for the elements match, we now have the full data from the elements array with the same name as in the theme.
    }
    //Loop through every element in the elements array.
  }
  //Loop through every element in the theme.
}
//This function actually processes the data, and applies the theme.
...
function error_handler(event) {
  if (evt.target.error.name == "NotReadableError") {
    chrome.tabs.query({
      ...
      chrome.tabs.executeScript(
        tabs[0].id, {
          code: 'alert("Cannot read file!");'
        });
    });
  }
}
//Gives the user an error if the file is not readable.

function get_as_text(fileToRead) {
  let reader = new FileReader();
  reader.readAsText(fileToRead);
  //This reades the CSV file.

  reader.onload = process_data;
  reader.onerror = error_handler;
  //Calls the appropiate function.
}
//The first function in the process, that readers the file and calls the process function.

document.getElementById("upload_theme").addEventListener("change", function () {
  if (window.FileReader) {
    get_as_text(document.getElementById("upload_theme").files[0]);
    //Calls the function als passes the uploaded file in as a parameter.

    file_uploaded = true;
  }
  //Checks if the browser supports file uploading.
  else {
    chrome.tabs.query({
      active: true,
      currentWindow: true
      ...
      chrome.tabs.executeScript(
        tabs[0].id, {
          code: 'alert("Your browser doesn\'t allow uploading files! You have to set your colors manually by looking in the CSV file.");'
        });
    });
  }
  //If not, give the user an error message.
});
//Adds the eventlistener for when a file is uploaded.
